...
export function AppEx(props) {
    ...
    const onAddDragPointerDown = (event: React.PointerEvent) => {
        ...
        (selfRef.current as Layout).addTabWithDragAndDrop(
            "Add text\n(Drag to location)",
            {
                component: "text",
    ...
    const onAddIndirectClick = (event: React.MouseEvent) => {
        (selfRef.current as Layout).addTabWithDragAndDropIndirect(
            "Add grid\n(Drag to location)",
            {
                component: "text",
                name: "Text" + nextGridIndex.current++,
    ...
    return (
        <div className='container'>
            <div>
                <button style={{ height: "30px", marginLeft: 5, border: "none", outline: "none" }} data-id='add-drag' onPointerDown={onAddDragPointerDown}>
                    Add Drag
                </button>
                <button data-id='add-active' onClick={onAddActiveClick}>
                    Add Active
                </button>
                <button data-id='add-indirect' onClick={onAddIndirectClick}>
                    Add Indirect
                </button>
                <button data-id='add-byId' onClick={onAddbyIdClick}>
                    Add by Id #1
                </button>
            </div>
            <div style={{ position: "relative", flexGrow: 1 }}>
                <Layout ref={selfRef} model={model} factory={factory} onRenderTab={onRenderTab} onRenderTabSet={onRenderTabSet} />
            </div>
        </div>
    );
}
